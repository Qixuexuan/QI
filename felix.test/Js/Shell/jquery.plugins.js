...
(function ($) {
    ...
    $.fn.tabs = function (options, param) {
        ...
        tabsContainer.bind('_resize', function () {
            if (tabsContainer.outerWidth() < tabsContainer.prop('scrollWidth')) {
                tabsLeftScroll.show();
    ...
    function addTab(param) {
        ...
        var html = '<div id="tabs_' + param.id + '" class="Tag"><a href="javascript:;" id="tabs_link_' + param.id + '" class="tab" index="' + param.id + '" closable="' + param.closable + '" hidefocus="hidefocus"><span class="TagLeft"></span><span class="TagMid">' + param.title + '</span><span class="TagRight"></span></a>';
        ...
        $('<div id="tabs_' + param.id + '_panel" class="tabs-panel">' + param.content + '</div>').appendTo(panelsContainer);
    ...
    function selectTab(id) {
        ...
        //  切换地图时，最小化窗体
        if (iframe.location.href.toLowerCase().indexOf("index.html") > -1) {
            if (ShowModelWindow) {
                ShowModelWindow();
            }
        }
        ...
        // 菜单表添加isfresh 存储过程里url后面添加isfresh参数
        if (iframe.location.href.toLowerCase().indexOf("isfresh=1") > -1) {
            //  针对   控件异步刷新
            if (iframe.refresh_datatable) {
                iframe.refresh_datatable();
            }
            else
