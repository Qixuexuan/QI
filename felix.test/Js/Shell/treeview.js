...
MzTreeView.prototype.nodeInit = function(sourceIndex, parentId)
{
  ...
  var exclude  = this.getAttribute(source, "exclude");
...
MzTreeView.prototype.nodeToHTML = function(node, AtEnd)
{
  ...
  var HTML = "<DIV nowrap='true'>";
  if(!isRoot)
  {
    node.childAppend = node.parentNode.childAppend + (AtEnd ? "　" : "│");
    if(this.wordLine)
    {
      HTML += "<SPAN nowrap='true'>"+ node.parentNode.childAppend + (AtEnd ? "└" : "├") +"</SPAN>";
      if(HCN) HTML += "<SPAN nowrap='true' class='pm' id='"+ this.name +"_expand_"+ id +"'>+</SPAN>";
    }
    else
    {
      node.iconExpand  = HCN ? AtEnd ? "PM2" : "PM1" : AtEnd ? "L2" : "L1";
      HTML += "<SPAN nowrap='true'>"+ this.word2image(node.parentNode.childAppend) +"<IMG "+
  ...
  HTML = "\r\n<SPAN nowrap='true' id='"+ this.name +"_tree_"+ id +"'>"+ HTML
  HTML +="<SPAN nowrap='true' style='DISPLAY: none'></SPAN></SPAN>";
  return HTML;
};
...
MzTreeView.prototype.initAll = function(nodes)
{
  for(var i=0;i<nodes.length;i++)
  {
     ...
     if(nodes[i].id==1 || nodes[i].type=="1" || nodes[i].exclude=="1")
        continue;
     ...
     span.innerHTML="&nbsp;<a href=\"javascript:parent.send_sms('"+user_id+"','"+ nodes[i].text +"')\" title='发短信' style='padding:0px;'><img src='/images/msg.png'></a>";
     span.innerHTML+="<a href=\"javascript:parent.send_email('"+ user_id +"','"+ nodes[i].text +"')\" title='发邮件' style='padding:0px;'><img src='/images/email.png'></a>";
     ...
     this.getElementById(this.name+"_link_"+nodes[i].id).onmouseover=function()
     {
        if(this.title!=this.innerText) return;
        var a=this.href.split('"');
        if(a[1]=="" || a[1]=="undefined") return;
        
        var id=this.id;
        if(typeof(jQuery) == 'undefined')
        {
           var req = new_req();
	        req.open("GET", "user_ip.php?SHOW_IP="+show_ip+"&USER_ID="+a[1], true);
	        req.onreadystatechange = function() { if (req.readyState == 4 && req.status==200) {$(id).title=req.responseText;$(id).onmouseover=function(){}}};
	        req.send('');
	     }
	     else
	     {
	        jQuery.ajax({
              type: 'GET',
              url: 'ipanel/user/user_ip.php',
              data: {'SHOW_IP': show_ip, 'USER_ID':a[1]},
